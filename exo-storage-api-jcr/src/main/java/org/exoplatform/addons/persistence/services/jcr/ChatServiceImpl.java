...
import org.exoplatform.addons.persistence.model.RoomBean;
import org.exoplatform.addons.persistence.model.RoomsBean;
import org.exoplatform.addons.persistence.model.SpaceBean;
...
import org.exoplatform.addons.persistence.utils.ChatUtils;
...
public class ChatServiceImpl extends AbstractJCRService implements ChatService
{
  ...
  public RoomsBean getRooms(String user, String filter, boolean withUsers, boolean withSpaces, boolean withPublic, boolean withOffline, boolean isAdmin, int limit, NotificationService notificationService, UserService userService, TokenService tokenService) {
    ...
    if (isAdmin)
      rooms.addAll(this.getExistingRooms(org.exoplatform.addons.persistence.services.UserService.SUPPORT_USER, withPublic, isAdmin, notificationService, tokenService));
    ...
    if (withUsers)
    {
      ...
      for (UserBean availableUser: availableUsers.values())
      {
        ...
        if (withOffline || (!withOffline && !org.exoplatform.addons.persistence.services.UserService.STATUS_INVISIBLE.equals(roomBean.getStatus()) && !org.exoplatform.addons.persistence.services.UserService.STATUS_OFFLINE.equals(roomBean.getStatus())))
        {
          rooms.add(roomBean);
        }
      }
